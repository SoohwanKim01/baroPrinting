...
if (isset($_POST['submit'])) {  //- [회원가입신청] 버튼을 클릭해 들어 왔으면
    ...
    if (!empty($uid) &&!empty($uname) && !empty($pass1) && !empty($pass2) && ($pass1 == $pass2) && !empty($captcha)) {
        ...
        if ($captcha <> $_SESSION['captcha']) {
            echo '<p class="text-danger">입력한 자동가입방지 글자가 틀립니다.  다시 입력하세요.</p>';
            echo '<p class="text-danger">captcha =' . $captcha . '</p>';
            echo '<p class="text-danger">session captcha =' . $_SESSION['captcha'] . '</p>';
        }
        else {
            ...
            if (mysqli_num_rows($data) == 0) {
                ...
                // 축하 메시지 주고 나간다.
                echo '<p class="text-success">회원가입을 축하합니다. 가입한 아이디로 <a href="login.php">[로그인]</a> 하세요.</p>';
                mysqli_close($dbc);
                exit();
            }
            else {
                // 이미 등록된 동일 아이디가 있으면..
                echo '<p class="text-danger">이미 사용중인 아이디 입니다. 다른 아이디를 사용하세요.</p>';
                $uid = "";
            }
        }
    }
    else {
        echo '<p class="text-danger">아이디,비번, 자동입력방지코드 중에서 빠진 것이 있습니다. 체크해 보세요.</p>';
    }
}
