...
if (isset($_POST['submit'])) { //설정확인
    if (empty($_POST['subject'])) { // 제목이 없으면
        ...
        $dbc = mysqli_connect(DB_HOST, DB_USER, DB_PASS, DB_NAME) or die(' DB접속 에러!');//DB연결
        mysqli_query($dbc, "SET NAMES UTF8");
        $subject = mysqli_real_escape_string($dbc, trim($_POST['subject'])); //제목 추가
        $content = mysqli_real_escape_string($dbc, trim($_POST['content'])); //내용 추가
